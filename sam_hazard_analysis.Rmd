...
fresno_parcels <- st_read("Fresno_Parcels_FeaturesToJSO.geojson")
fb_parcels <- 
  fresno_parcels %>% 
...
street_edges <-
  blocks %>% 
  st_difference(blocks %>% st_buffer(-5)) %>% # 5 for front yard considerations
...
yards_setbacks <-
  ...
  mutate(lot_area = st_area(.)) %>% 
  arrange(desc(lot_area)) %>%  
  st_buffer(-17) %>% # to account for streets 12 + 5 ft for front yard
  st_difference(st_union(bldg_osm %>% st_buffer(14))) %>% # according to R-2 zoning design + 4 for parcel boundary
